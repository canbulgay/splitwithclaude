...
export function DashboardPage() {
  ...
  const [activityData, setActivityData] = useState<Array<{id: string; type: 'expense' | 'settlement' | 'group' | 'balance'; title: string; description: string; amount?: number; timestamp: Date; groupName?: string; userName?: string}>>([]);
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `${recentExpenses.reduce((sum, expense) => sum + expense.amount, 0).toFixed(2)}`,
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
          ...
          try {
            ...
            groupExpenses.expenses.forEach(expense => {
              activities.push({
                ...
                description: `Added by ${(expense.paidBy as any)?.name || 'Unknown'}`,
        ...
        const chartDataArray = Object.entries(categoryData).map(([name, value]) => ({
        ...
        {recentExpenses.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your latest expense activity across all groups</CardDescription>
            </CardHeader>
            <CardContent>
              <ExpenseList
