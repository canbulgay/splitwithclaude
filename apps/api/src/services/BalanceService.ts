...
export class BalanceService {
   ...
   */
  static async calculateGroupBalancesRealTime(
    groupId: string
  ): Promise<Balance[]> {
   ...
   */
  static async calculateUserBalanceSummary(
    userId: string
   ...
   */
  static async optimizeSettlements(
    groupId: string
  ): Promise<SettlementSuggestion[]> {
   ...
   */
  static async calculateGroupBalancesWithSettlements(
    groupId: string
  ): Promise<Balance[]> {
    try {
      // Get original balances from expenses
      const expenseBalances = await ExpenseModel.calculateGroupBalances(
        groupId
      );

      // Get all confirmed settlements between group members
      const confirmedSettlements = await SettlementModel.findByGroupMembers(
        groupId
      );
   ...
   */
  static async getActiveSettlementSuggestions(
    groupId: string
  ): Promise<SettlementSuggestion[]> {
    try {
      const outstandingBalances =
        await this.calculateGroupBalancesWithSettlements(groupId);
   ...
   */
  private static optimizeBalanceArray(
    balances: Balance[]
